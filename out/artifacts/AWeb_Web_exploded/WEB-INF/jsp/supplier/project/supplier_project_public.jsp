...
<jsp:include page="../header.jsp" />
  ...
  <div id="contents">
    <div class="inner">
    	...
    	<jsp:param name="menu" value="project_public" />
    	</jsp:include>
                  ...
                  <div class="form-group">
                     ...
                     <div class="input_area">
                       <select class="form-control" id="auditStatus">
                         <option value="">不限</option>
                         <option value="0"<c:if test="${auditStatus eq '0'}"> selected="selected"</c:if>>未参与</option>
                         <option value="1"<c:if test="${auditStatus eq '1'}"> selected="selected"</c:if>>待审核</option>
                         <option value="3"<c:if test="${auditStatus eq '3'}"> selected="selected"</c:if>>审核失败</option>
                       </select>
                     </div>
                  </div>
                  <div class="form-group">
            ...
            <table class="table_area">
              ...
              <c:forEach items="${data}" var="item">
	             	<tr>
		                ...
		                <td>
		                	<c:if test="${empty item.auditStatus}">
		                		未参与
		                	</c:if>
		                ...
		                <td>
		                	<c:if test="${empty item.auditStatus}">
		                		<a href="javascript:void(0);" class="join" data-id="${item.projId}" data-name="${item.projName}">报名</a>
		                	</c:if>
		                </td>
		            </tr>
              </c:forEach>
...
$(function () {
	$('#search').click(function () {
		...
		String projType,
			...
			String endTime
		*/
		location.href = '${pageContext.request.contextPath}/supplier/project/public?projType=' + $('#projType').val()
	...
	$('.join').click(function () {
		if (confirm('确认报名【' + $(this).attr('data-name') + '】项目？')) {
			var projId = $(this).attr('data-id');
			$.post('${pageContext.request.contextPath}/supplier/project/join_project', {projId : projId}, function (res) {
				if (res.status == 1) {
					alert(res.result);
				} else {
					alert(res.result);
