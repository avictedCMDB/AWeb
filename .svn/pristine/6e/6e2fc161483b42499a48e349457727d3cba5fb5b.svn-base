...
function CreateWebSignObject(objectIDString, OnSignCallbackFun, OnSignCallbackFunString, OnVerifyCallbackFun, OnVerifyCallbackFunString, OnSignRemovedCallbackFun, OnSignRemovedCallbackFunString) {	
	...
	o.GetCerInfo = function (type) {
		return clt.GetCerInfo(type);
	};
	...
	o.GetESealFromSignature = function (data) {
		return clt.GetESealFromSignature(data);
	};
	...
	try {
		if ($checkBrowserISIE()) {	// IE
			if(window.navigator.platform == "Win32")   //codeBase="BJCAWebSign.CAB#version=4,1,0,0"
				pluginDiv.innerHTML = '<object id="' + ctlName + '" classid="CLSID:'+ CLSID +'" codeBase="' + cabPath + '/BJCAWebSign.CAB#version=4,2,1,2" style="POSITION: absolute; TOP: 10px; LEFT: 10px;"> <PARAM NAME="Visible" VALUE="false"> <PARAM NAME="Editable" VALUE="false"> </object>';
			else
				pluginDiv.innerHTML = '<object id="' + ctlName + '" classid="CLSID:'+ CLSID +'" codeBase="' + cabPath + '/BJCAWebSignX64.CAB#version=4,2,1,2" style="POSITION: absolute; TOP: 10px; LEFT: 10px;"> <PARAM NAME="Visible" VALUE="false"> <PARAM NAME="Editable" VALUE="false"> </object>';
			if (addEvent) {
