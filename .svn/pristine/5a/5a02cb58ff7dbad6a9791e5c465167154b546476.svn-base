...
$(function () {
	...
	$("button[data-step]").click(function () {
		...
		if (cur == 1) {
			...
			$("#mtr_sel").append('<label class="checkbox-inline"><input id="mtr_check_all" type="checkbox" onclick="mtrCheckAll(this)">全选</label>');
			mtrs.each(function () {
				...
				param.materiels.push({
					...
					materPrice:tds.eq(5).text(),
					ifStandard:tds.eq(6).text(),
					saleService:tds.eq(7).text(),
					specConf:tds.eq(8).text()
				});
                $("#mtr_sel").append('<label class="checkbox-inline"><input type="checkbox" onclick="mtrCheck(this)" name="mtr_sel" id="mtr_sel_' + $(this).attr("data-id") + '" value="' + $(this).attr("data-id") + '">' + $(this).find("td").eq(0).text() + '</label>');
			});	
			}
			...
			if ($("#proj_type").val() == 1) {
				...
				if (suppliers.size() < param.minTender) {
					alert("请添加最少" + param.minTender + "家供应商");
					return false;
				}
			...
			if (min_incr_type == 3) {
				if (param.quotaLimitType != 2) {
					alert("报价上限为不限时不能选择最低降价幅度（总价百分比）");
					return false;
				}
			...
			var show_min_quota = $("input[name=show_min_quota]:checked").val();
			param.showMinQuota = show_min_quota;
			}
	...
	$("#mtr_query_add").click(function () {
		...
		sel.each(function () {
			...
			mtrInsert({
				...
				materPrice:chx.attr("data-materPrice"),
	...
	$("#subject_add").click(function () {
		...
		async(CONTEXT + '/management/bid/' + ACT + '/subject/no', mtrParam, function (msg) {
			...
			$("#subject_tbody").children().each(function () {
				var data_mtr = $(this).attr("data-mtr");
				$(data_mtr.split(",")).each(function () {
					$("#mtr_sel_" + this).attr("disabled", "disabled");
				});
			});
			

			$("#mtr_check_all")[0].checked = false;
	...
	$("#subject_modal_ok").click(function () {
		...
		if (subject_num == "") {
			alert("请输入数量");
			return;
		} else if (!/^\d+([.]\d+)?$/.test(subject_num) || subject_num < 1) {
			alert("数量必须为大于1的数字");
			return;
		}
		...
		if (result.data) {
            ...
            */
          	...
          	for (var i = 0; i < result.data.length; i++) {
	            ...
	            chx.attr("data-materPrice", item.materPrice);
	            var td = $("<td>");
	            td.append(chx);
	            tr.append(td);
...
var subjectEdit = function (obj) {
	...
	$(row.attr("data-mtr").split(",")).each(function () {
		if ($("#mtr_sel_" + this).size() > 0) {
			...
			$("#mtr_sel_" + this).removeAttr("disabled");
		}
	});
	...
	var allCheck = false;
	$("input[name=mtr_sel]").each(function () {if (this.disabled) return; allCheck = this.checked; if (!this.checked) {allCheck = false; return false;}});
	if (allCheck) {
		$("#mtr_check_all")[0].checked = true;
	}
...
var mtrCheckAll = function (obj) {
	if (obj.checked) {
		$("input[name=mtr_sel]").each(function () {if (this.disabled) return; this.checked = true;});
	} else {
		$("input[name=mtr_sel]").removeAttr("checked");
	}
}

var mtrCheck = function (obj) {
	if (obj.checked) {
		var allCheck = false;
		$("input[name=mtr_sel]").each(function () {if (this.disabled) return; allCheck = this.checked; if (!this.checked) {allCheck = false; return false;}});
		if (allCheck) {
