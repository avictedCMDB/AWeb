...
public class JdServiceAdapter extends ServiceAdapter {
    ...
    private static final String URL_BIZ_API = "https://bizapi.jd.com/api";
    ...
    public Map<String, Object> fetchSku(int cat, int page) throws ServiceExecutionFailedException, TokenInvalidException {
        logger.info("京东-获取SKU [" + cat + "] page [" + page + "]");
        ...
        try {
            ...
            logger.info("京东-SKU获取成功，[" + cat + "] page [" + page + "] 数量： [" + list.size() + "]");
        } catch (TokenInvalidException ex) {
            throw ex;
        } catch (Exception e) {
            logger.error("京东-SKU获取失败[" + cat + "] page [" + page + "]", e);
            throw new ServiceExecutionFailedException(e);
        }
        rtn.put("list", skus);
        return rtn;
    ...
    public Map<String, Price> fetchPrice(List<String> skus) throws ServiceExecutionFailedException, TokenInvalidException {
        ...
        try {
            ...
            for (Map<String, Object> i : list) {
                ...
                if(subMsg1.contains(String.valueOf(i.get("skuId"))) || p > price.getSalePrice()){
        ...
        */
        ...
        try {
            ...
            if ((result.containsKey("code") && "0".equals(result.get("code"))) && (!result.containsKey("success") || "true".equals(String.valueOf(result.get("success"))))) {
                return result;
            } else if("biz.afterSale.auditCancel.query".equals(method) && result.get("resultCode") == null && "false".equals(String.valueOf(result.get("success")))){
    ...
    public void fetchBizToken() throws Exception, TokenInvalidException {
        ObjectMapper mapper = new ObjectMapper();
        CloseableHttpResponse response = null;
        HttpEntity entity = null;
        Map<String, String> param = new HashMap<String, String>();
        param.put("grant_type", "access_token");
        param.put("client_id", supplier.getCompanyCode());
        param.put("client_secret", supplier.getDeptCode());
        param.put("timestamp", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
        param.put("username", supplier.getAuthUser());
        param.put("password", supplier.getAuthPass());

        String all = param.get("client_secret") + param.get("timestamp") + param.get("client_id") + param.get("username") + param.get("password") + param.get("grant_type") + param.get("client_secret");
        param.put("sign", MD5.getMessageDigest(all.getBytes("UTF-8")).toUpperCase());
        ...
        req = buildRequest("https://bizapi.jd.com/oauth2/accessToken", new UrlEncodedFormEntity(nv, "UTF-8"));

        logger.debug("京东-接口调用[https://bizapi.jd.com/oauth2/accessToken] 参数 : " + mapper.writeValueAsString(param));
        ...
        try {
            ...
            content = EntityUtils.toString(entity, "UTF-8");
            logger.debug("京东-接口调用成功 [https://bizapi.jd.com/oauth2/accessToken] 返回 : " + content);
            ...
            if ("true".equals(String.valueOf(result.get("success")))) {
                ...
                this.bizToken = (String)((Map<String, Object>)result.get("result")).get("access_token");
            } else {
            ...
            logger.error("京东-接口调用失败 [https://bizapi.jd.com/oauth2/accessToken]", e);
            throw e;
        } finally {
            try {
                response.close();
    ...
    private Map<String, Object> executeBizApi(String method, Map<String, String> param) throws Exception, TokenInvalidException {
        ObjectMapper mapper = new ObjectMapper();
        CloseableHttpResponse response = null;
        HttpEntity entity = null;
        
        param.put("token", bizToken);
        ...
        req = buildRequest(URL_BIZ_API + method, new UrlEncodedFormEntity(nv, "UTF-8"));

        logger.debug("京东-接口调用[" + method + "] 参数 : " + mapper.writeValueAsString(param));
	...
	public Map<String, Object> afterSaleAfsApplyCreate(Map afsApplyCreate)
			throws ServiceExecutionFailedException, TokenInvalidException {
		 	logger.info("京东-服务单保存申请");
		 	//组装请求参数
	        Map<String, Object> param = new HashMap<String, Object>();
	        param.put("param",  JSONObject.fromObject(afsApplyCreate));
	        
	        Map<String, Object> result;
	        try {
	        	result = executeApi("biz.afterSale.afsApply.create", param);
	        	System.out.println("返回json数据："+JSONObject.fromObject(result).toString());
	            logger.info("京东-服务单保存申请");
	        } catch (TokenInvalidException ex) {
	            throw ex;
	        } catch (Exception e) {
	            logger.error("京东-服务单保存申请失败", e);
	            throw new ServiceExecutionFailedException(e);
	        }
	        return result;
	}
     ...
     */
	@Override
	public Map<String, Object> serviceListPage(String jdOrderId,int pageIndex,int pageSize)
			throws ServiceExecutionFailedException, TokenInvalidException {
		 	logger.info("京东-订单号分页查询服务单概要信息");
		 	//组装请求参数
	        Map<String, Object> param = new HashMap<String, Object>();
	        param.put("jdOrderId",  jdOrderId);
	        param.put("pageIndex",  pageIndex);
	        param.put("pageSize",  pageSize);
	        Map<String, Object> jdparam = new HashMap<String, Object>();
	        jdparam.put("param", JSONObject.fromObject(param).toString());
	        Map<String, Object> result;
	        try {
	        	result = executeApi("biz.afterSale.serviceListPage.query", jdparam);
	        	System.out.println("返回json数据："+JSONObject.fromObject(result).toString());
	            logger.info("京东-订单号分页查询服务单概要信息");
	        } catch (TokenInvalidException ex) {
	            throw ex;
	        } catch (Exception e) {
	            logger.error("京东-订单号分页查询服务单概要信息失败", e);
	            throw new ServiceExecutionFailedException(e);
	        }
	        return result;
	}
	...
	public Map<String, Object> auditCancel(Map auditCancel)
			throws ServiceExecutionFailedException, TokenInvalidException {
		logger.info("京东-取消服务单/客户放弃");
	 	//组装请求参数
        Map<String, Object> param = new HashMap<String, Object>();
        param.put("param",  JSONObject.fromObject(auditCancel));
        
        Map<String, Object> result;
        try {
        	result = executeApi("biz.afterSale.auditCancel.query", param);
        	System.out.println("返回json数据："+JSONObject.fromObject(result).toString());
            logger.info("京东-取消服务单/客户放弃");
        } catch (TokenInvalidException ex) {
            throw ex;
        } catch (Exception e) {
            logger.error("京东-取消服务单/客户放弃失败", e);
            throw new ServiceExecutionFailedException(e);
        }
        return result;
	}
	...
	public void delMessage(String msg) throws ServiceExecutionFailedException, TokenInvalidException {
        ...
        logger.info("京东-删除推送消息[" + msg + "]");
        Map<String, String> param = new HashMap<String, String>();
        param.put("id", msg);
        try {
            executeApi("biz.message.del", param);
            logger.info("京东-删除推送消息成功，消息[" + msg + "]");
            
        } catch (TokenInvalidException ex) {
            throw ex;
        } catch (Exception e) {
            logger.error("京东-删除推送消息失败", e);
            throw new ServiceExecutionFailedException(e);
        }
	}
	...
	public List<GoodsMessage> getMessage() throws ServiceExecutionFailedException, TokenInvalidException {
	    logger.info("京东-获取推送消息");

        Map<String, String> param = new HashMap<String, String>();
        param.put("type", "2,4,6,16");
        ...
        try {
            ...
            result = executeApi("biz.message.get", param);
            list = (List<Map<String, Object>>) result.get("result");
            if (list == null) {
                throw new ServiceExecutionFailedException("无数据");
            }
            ...
            for (Map<String, Object> map : list) {
                ...
                if ("2".equals(type)) {
                    ...
                    msg.setOperation(GoodsMessage.Operation.STATE_CHANGE);
            ...
            logger.info("京东-获取推送消息成功，数量[" + msgs.size() + "]");
            return msgs;
            
        } catch (TokenInvalidException ex) {
            throw ex;
        } catch (Exception e) {
            logger.error("京东-获取推送消息失败", e);
            throw new ServiceExecutionFailedException(e);
        }
	}
}
