...
public class InvoiceController {
    ...
    @RequestMapping("/add")
    public String addInvoice(Map<String, Object> map,@ModelAttribute("invoice") Invoice invoice,HttpSession httpSession) {
        logger.info("添加发票信息");
        User user = new User();
        try {
        	user = Permission.check(httpSession, UserRole.BUY,UserRole.CLIENT);
		...
		if (!(invoice.getOldCompanyName().equals(invoice.getCompanyName()) && invoice.getOldInvoiceNum().equals(invoice.getInvoiceNum()))) {
			int count = invoiceService.checkInvoiceEx(invoice);
			if (count != 0) {
				map.put("invoice", invoice);
				map.put("msg", "纳税人识别号已存在！请重新输入!");
		        return "management_invoice_add";
			}
		}
    ...
    public String updateInvoice(Map<String, Object> map,@ModelAttribute("invoice") Invoice invoice,HttpSession httpSession) {
        logger.info("修改发票信息");
        User user = new User();
        try {
        	user = Permission.check(httpSession, UserRole.BUY,UserRole.CLIENT);
		...
		if (!(invoice.getCompanyName().equals(invoice.getOldCompanyName()) && invoice.getInvoiceNum().equals(invoice.getOldInvoiceNum()))) {
			int count = invoiceService.checkInvoiceEx(invoice);
			if (count != 0) {
				map.put("invoice", invoice);
				map.put("msg", "纳税人识别号已存在！请重新输入!");
