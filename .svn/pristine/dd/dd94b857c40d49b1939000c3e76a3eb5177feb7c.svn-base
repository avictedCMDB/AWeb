...
$(function () {
	...
	$(".submit-btn").click(function () {
		...
		if (confirm("是否确认" + $(this).text() + "?")) {
			...
			async('${pageContext.request.contextPath}/management/order/doContractAudit', {order_id:'${order.order_id}', contract_code:'${contract.contract_code}', contractStatus:$(this).attr("data-val"), contractRemark:contractRemark}, function (msg) {
				alert("提交成功");
  ...
  <div id="contents">
    <div class="inner">
      ...
      <div id="matter_section">
              <h2>采购合同</h2>
              <p class="code">合同编号：${contract.contract_code}</p>
              <div class="summary">
                <p><span class="border_bottom">${user.companyName }</span>（以下简称“甲方”）与 <span class="border_bottom">${order.sup_name }</span>（以下简称“乙方”）在平等、自愿、公平、诚实信用之基础上，就甲方<span class="border_bottom">${contract.contract_name}</span>项目（项目编号<span class="border_bottom">${order.order_id }</span>）向乙方采购<span class="border_bottom">${subs }</span>事宜，经友好协商一致，订立本合同，以资共同信守。</p>
              </div>
              ...
              <div class="signo clearfix">
	            ...
	            <div class="section list_notice suggestion">
	              <h3>审核意见</h3>
	              <textarea class="form-control" id="contract_remark" <c:if test="${order.order_status != 24}">disabled="disabled"</c:if>>${contract.remark}</textarea></div>
	            
	            
				<div class="submit_btn">
	                 <ul>
	              <c:if test="${order.order_status == 24}">
	                    <li><button class="btn btn-default blue submit-btn" data-val="21">通过</button></li>
	                    <li><button class="btn btn-default gray_cart submit-btn" data-val="25">拒绝</button></li>
	              </c:if>
