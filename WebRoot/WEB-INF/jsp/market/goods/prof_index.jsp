...
$(function () {
	$(".p1-cat").change(function () {
		...
		sup.html('<option value="">全部</option>');
		
		if (cat != "") {
			brand.html('<option value="">加载中...</option>');
			async('${pageContext.request.contextPath}/market/goods/prof/brand', {cat:cat}, function (msg) {
				...
				brand.html("<option value=''>加载失败</option>");
			});
		}		
	});
	$(".p1-brand").change(function () {
		...
		if (brand != "") {
			sup.html('<option value="">加载中...</option>');
			async('${pageContext.request.contextPath}/market/goods/prof/sup?cat=' + cat.val() + '&brand=' + encodeURIComponentGBK(brand), {}, function (msg) {
				...
				sup.html("<option value=''>加载失败</option>");
			});
		}
	});
                    	...
                    	<li class="search_main_box" <c:if test="${sta.index > 0}"> style="display:none;"</c:if>>
                            ...
                            <c:forEach var="item" items="${p1.subs}" varStatus="sta2">  
                                <div class="search_input_box" data-cat1="${p1.marketCatId}" data-id="${item.marketCatId}" <c:if test="${sta2.index > 0}"> style="display:none;"</c:if>>
                                    ...
                                    <div class="btn_search"><button class="btn btn-default blue p1-search" type="button">搜索</button><a href="${pageContext.request.contextPath}/management/specialGoods" class="btn btn-default blue" style="margin-left:20px;" >求购</a></div>
                    ...
                    <div class="supplier_area_bottom">
                        <div class="left_area">
                            ...
                            <c:forEach var="item" items="${newSup}">                            
                                <li><a href="${pageContext.request.contextPath}/market/goods/prof?sup=${item.supId}&from=sup">${item.supName}</a><span><fmt:formatDate value="${item.enterDate}" pattern="yyyy-MM-dd"/>入驻</span></li>
                            </c:forEach>
                            </ul>
                        </div>
                        <div class="right_area">
                            ...
                            <c:forEach var="item" items="${recSup}">
                                <li>
                                    ...
                                    <p class="gray">经营品类：${item.busiType}</p>
                                    <p class="gray">经营品牌：${item.busiRange}</p>
                                </li>
                            </c:forEach>
                            </ul>
                        </div>
