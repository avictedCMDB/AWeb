                	...
                	<c:when test="${s.ifCancel==0}">
                	    <c:choose>
                	    	<c:when test="${service.exchangeOrRefound=='1'}">
                	    		<td class="last"><a href="javascript:void(0);" onclick="auditCancle('${s.servicesID}','${s.goodsPrice}','${s.goodsPriceInit}','${s.exchangeNum}','${s.supID}','${s.typeID}','${s.supGoodsID}','${orderID}');">取消  </a></td>
                	    	</c:when>
                	    	<c:when test="${service.exchangeOrRefound=='2'}">
                	    		<td class="last"><a href="javascript:void(0);" onclick="auditCancle('${s.servicesID}','${s.goodsPrice}','${s.goodsPriceInit}','${s.refoundNum}','${s.supID}','${s.typeID}','${s.supGoodsID}','${s.orderID}');");">取消</a></td>
                	    	</c:when>
                	    </c:choose>                		
                	</c:when>
                    <c:otherwise>
