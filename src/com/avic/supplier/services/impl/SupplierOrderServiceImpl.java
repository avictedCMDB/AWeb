...
public class SupplierOrderServiceImpl implements SupplierOrderService {
			...
			for (String orderId : orderIdMap.keySet()) {
				...
				for (int j = 0; j < orderList.size(); j++) {
					if (orderId.equals(orderList.get(j).getOrderId())) {
						...
						order.setContractCode(orderList.get(j).getContractCode());
						// 转化订单状态
						if (Integer.valueOf(orderList.get(j).getOrderStatus()) >= 7 
								...
								&& !"22".equals(orderList.get(j).getOrderStatus())
								&& !"23".equals(orderList.get(j).getOrderStatus())
								&& !"24".equals(orderList.get(j).getOrderStatus())
								&& !"25".equals(orderList.get(j).getOrderStatus())) {
	...
	public void insertContract(SupplierOrderContract supplierOrderContract) {
		supplierOrderMapper.insertContract(supplierOrderContract);
	}
	@Override
	public void insertImageInfo(SupplierImageInfo supplierImageInfo) {
		supplierOrderMapper.insertImageInfo(supplierImageInfo);
	}
	@Override
	public SupplierOrderContract queryOrderContract(String orderId) {
		return supplierOrderMapper.queryOrderContract(orderId);
	}
	@Override
	public List<SupplierImageInfo> queryImageInfo(String contractCode,String businessType) {
		return supplierOrderMapper.queryImageInfo(contractCode,businessType);
	}
	@Override
	public List<SupplierQuoteInfo> queryQuoteInfo(String orderId) {
		return supplierOrderMapper.queryQuoteInfo(orderId);
	}
	@Override
	public void updataQuoteInfo(SupplierQuoteInfo supplierQuoteInfo) {
		supplierOrderMapper.updataQuoteInfo(supplierQuoteInfo);
