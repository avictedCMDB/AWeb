...
public class CompanyInfoServiceImpl implements CompanyInfoService {
	...
	public void deleteSupInfo(CompanyInfo companyInfo) {
		companyInfoMapper.deleteSupInfo(companyInfo);
	}	
	...
	public int getSupplierCompanyInfoCount(CompanyInfo companyInfo) {
		return companyInfoMapper.getSupplierCompanyInfoCount(companyInfo);
	}

	@Override
	public List<OrganizationInfoNode> queryOrganizationInfoNodes(
			CompanyInfo companyInfo) {
		CompanyInfo ci = new CompanyInfo();
		List<CompanyInfo> companys = new ArrayList<CompanyInfo>();
		companys = companyInfoMapper.queryDepartments(companyInfo);
		List<OrganizationInfoNode> nodes = new ArrayList<OrganizationInfoNode>();
		nodes = queryOrganizationInfos(companys,companyInfo.getCompanyCode());
		//ci = recursiveTreeDepartment(companyInfo.getCompanyCode(),companys);
		return nodes;
	}
	
	public List<OrganizationInfoNode> queryOrganizationInfos(List<CompanyInfo> companyInfos,String companyInfoCode) {
		// 节点列表（散列表，用于临时存储节点对象）
		HashMap nodeList = new HashMap();
		// 根据结果集构造节点列表（存入散列表）
		for (Iterator it = companyInfos.iterator(); it.hasNext();) {
			CompanyInfo dataRecord = (CompanyInfo) it.next();
			OrganizationInfoNode node = new OrganizationInfoNode();
			node.id = dataRecord.getCompanyCode();
			node.text = dataRecord.getCompanyName();
			node.parentId = dataRecord.getParentCompanyId();
			node.menuLevel = dataRecord.getLevel();
		...
		// 构造无序的多叉树
		Set entrySet = nodeList.entrySet();
		List<OrganizationInfoNode> rootList =  new ArrayList<OrganizationInfoNode>();
		for (Iterator it = entrySet.iterator(); it.hasNext();) {
			OrganizationInfoNode node = (OrganizationInfoNode) ((Map.Entry) it.next()).getValue();
			if (companyInfoCode.equals(node.id)) {
				rootList.add(node);
			} else {
				((OrganizationInfoNode) nodeList.get(node.parentId)).addChild(node);
			}
		}
		for (int i = 0; i < rootList.size(); i++) {
			rootList.get(i).sortChildren();
	...
	public int queryOrganizationUserCount(CompanyInfo companyInfo) {
		return companyInfoMapper.queryOrganizationUserCount(companyInfo);
	}

	@Override
	public int queryOrganizationDepartmentCount(CompanyInfo companyInfo) {
		return companyInfoMapper.queryOrganizationDepartmentCount(companyInfo);
	}

	@Override
	public void deleteCompanyDepartments(CompanyInfo companyInfo) {
		companyInfoMapper.deleteCompanyDepartments(companyInfo);
	}

	@Override
	public void updateSupUserPassword(String password, String supID) {
		companyInfoMapper.updateSupUserPassword(password, supID);
	}
}
